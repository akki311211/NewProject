...
public class MultipartParser {
   ...
   */
  public MultipartParser(HttpServletRequest request, int maxSize, boolean buffer, 
                         boolean limitLength) throws IOException {
    this(request, maxSize, buffer, limitLength, null);
  }
   ...
   *                    the request's input stream to prevent trying to 
   *                    read past the end of the stream.
   * @param encoding the encoding to use for parsing, default is utf-8.
   */
  public MultipartParser(HttpServletRequest request, int maxSize, boolean buffer, 
    ...
    //if(NewPortalUtils.checkUploadLimit(request))// P_TR_ENH_AmazonSkyConfig ,P_INT_ENH_AmazonSkyConfig 
    if(BaseUtils.checkUploadLimit(request))// P_TR_ENH_AmazonSkyConfig ,P_INT_ENH_AmazonSkyConfig 
     {	
	    if (length > maxSize) {
	      throw new IOException("Posted content length of " + length + 
	                            " exceeds limit of " + maxSize);
	    }
    }// P_TR_ENH_AmazonSkyConfig ,P_INT_ENH_AmazonSkyConfig 
...
//}
  ...
  /**
   * This method same as MultipartParser(HttpServletRequest request, int maxSize, boolean buffer, 
   * boolean limitLength, String encoding)
   * but this method does not restrict request length which helps to parse only the request parameters 
   * this method is only used by FCMultipartRequestWrapper to extract parameters only from request
   * 
   * Creates a <code>MultipartParser</code> from the specified request,
   * optionally buffers for performance and prevents attempts to read past the amount 
   * specified by the Content-Length, and with a specified encoding. 
   * 
   * @param request   the servlet request.
   * @param maxSize the maximum size of the POST content.
   ...
   */
	...
	public MultipartParser(HttpServletRequest request,boolean buffer, boolean limitLength, String encoding)
			throws IOException {
		// First make sure we know the encoding to handle chars correctly.
		// Thanks to Andreas Granzer, andreas.granzer@wave-solutions.com,
		// for pointing out the need to have this in the constructor.
  ...
  /**
   * Must call this method when process is completed 
   */
  ...
  public void close(){
      try{
          ...
          logger.error("exception in MultipartParser---", e);
          in = null;
      }
  }
   ...
   * utf-8.  Encodings are actually best passed into the contructor,
