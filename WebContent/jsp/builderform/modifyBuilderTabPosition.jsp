<!--
 - $Header: web\jsp\fim\modifyBuilderTabPosition.jsp
 - $Author: akashg $
 - $Date: 2016/10/12 05:43:02 $

 -
 - ====================================================================
 - Basic Description/Data Flow of JSP
 - Tab reordering in formbuilder
 -->
...
try {
	...
	String subModuleName=request.getParameter("subModuleName");
	 request.setAttribute(FieldNames.MODULE,moduleName);
	 request.setAttribute("subModuleName",subModuleName);
	boolean flag1 = false;
	 if(StringUtil.isValidNew(actionTaken) && EventType.MODIFY.equals(actionTaken)) {
		flag1 = builderForm.addModifyBuilderTabsOrder(request);
	} 
		tabGeneratorMap=builderForm.getBuilderTabMap(request);
		List configuredTabList = null;
		if("fim".equals(moduleName))
		{
			configuredTabList = builderForm.getConfiguredTab(subModuleName);
		}
%>
<%@ page import="com.appnetix.app.util.*"%>
...
<%
		for (int i=0; i<tabGeneratorMap.size(); i++) {%>
			if($('#sortable li').get(<%=i%>)) {
 				document.changeForm.<%=BuilderFormFieldNames.TAB_DISPLAY_NAME%>_<%=i%>.value =  $('#sortable li').get(<%=i%>).lang; 
				document.changeForm.<%=BuilderFormFieldNames.TAB_DISPLAY_NAME%>_<%=i%>_<%=NO%>.value= <%=i%>;
				document.changeForm.<%=BuilderFormFieldNames.TAB_DISPLAY_NAME%>_<%=i%>_<%=BuilderFormFieldNames.IS_CUSTOM%>.value= $('#sortable li').get(<%=i%>).id;;
			}
<%}%>
		f.action = "modifyBuilderTabPosition";
		f.submit();
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	<tr>
		...
		<%} %>
		&gt; <a linkindex="19" href="builderWebForm?moduleId=<%=moduleId%>&module=<%=moduleName%>"><u><%=LanguageUtil.getString("Manage Form Generator", (String) session.getAttribute("userLanguage"))%></u></a>
		&gt; <span class="text_b"><%=LanguageUtil.getString("Modify Tabs Position", (String) session.getAttribute("userLanguage"))%></span></td>
	</tr>
	<tr>
	...
	<tr>
			<td width="10"><img src="<%=request.getContextPath()%>/static<%=Constants.STATIC_KEY%>/images/spacer.gif" width="10" height="1"></td>
			<td width="100%" height="10" align = "right" class = "bText11gr"><span class = "urgent_fields">*</span> <%=LanguageUtil.getString("Use Drag and Drop pattern to change any Tab's Sequence")%></td>
			<td width="10"><img src="<%=request.getContextPath()%>/static<%=Constants.STATIC_KEY%>/images/spacer.gif" width="10" height="1"></td>
	</tr>
	...
	<input type="hidden" name="fimFormSize" value=<%=tabGeneratorMap.size()%>>
	<%
		for (int i = 0; i < tabGeneratorMap.size(); i++) { 
	%> 
	<input type="hidden" name="<%=BuilderFormFieldNames.TAB_DISPLAY_NAME%>_<%=i%>">
	<input type="hidden" name="<%=BuilderFormFieldNames.TAB_DISPLAY_NAME%>_<%=i%>_<%=NO%>">
	<input type="hidden" name="<%=BuilderFormFieldNames.TAB_DISPLAY_NAME%>_<%=i%>_<%=BuilderFormFieldNames.IS_CUSTOM%>">
	<%
		}
	%> <input type="hidden" name="<%=FieldNames.ERROR_PAGE%>" value="errorPage"> <input type="hidden" name="<%=FieldNames.RETURN_PAGE%>" value="modifyBuilderTabPosition">
        ...
        <td width="100%">
            <table border="0" cellpadding="0" cellspacing="0" width="100%">
               ...
               <td class="thead hText16black"></td>
                    <td width="100%" nowrap="nowrap" class="thead hText16black"><%=LanguageUtil.getString("Available Tabs")%> </td>
                    <td class="thead hText16black"></td>
                  </tr>
                </table>
                    ...
                    <tr>
                      ...
                      <div id="abhi" style="padding:0px 4px 4px 2px;">
                          ...
                          <%
										...
										Iterator idSetIterator = tabGeneratorMap.keys().iterator();
										String tabNo = "";
										String displayName = "";
										...
										while (idSetIterator.hasNext()) {
											...
											Info currentInfo = (Info) tabGeneratorMap.get(key);
											//tabNo = currentInfo.getString("No");
											displayName = currentInfo.getString(BuilderFormFieldNames.DISPLAY_NAME);
											displayTitle = PortalUtils.filterValue(displayName);
											tabName = currentInfo.getString(BuilderFormFieldNames.TAB_DISPLAY_NAME);
											isCustom=currentInfo.getString(BuilderFormFieldNames.IS_CUSTOM);
											if("fim".equals(moduleName) && !configuredTabList.contains(displayName) && "N".equals(isCustom))
											{
												display = "none";
											}
											isActive = currentInfo.getString(BuilderFormFieldNames.TAB_IS_ACTIVE);
											if("N".equals(isActive)) {
												continue;
											}
											
											if(Constants.PM_TAB_DISPLAY_NAME.equals(displayName) && ("off".equalsIgnoreCase(MultiTenancyUtil.getTenantConstants().PM_INTEGRATION_STATUS) || ("on".equalsIgnoreCase(MultiTenancyUtil.getTenantConstants().PM_INTEGRATION_STATUS) && !StringUtil.isValidWithZero(MultiTenancyUtil.getTenantConstants().PROVEN_MATCH_FRANCHISOR_ID)))){
    											continue;
    										}
											
											if("Local Listing".equals(displayName) && !"Y".equals(com.appnetix.app.components.masterdatamgr.manager.MasterDataMgr.newInstance().getMasterDataDAO().getValueById("1774572525"))) {
												continue;
											}
											if("Multi-Unit Info".equals(tabName)){
												displayName=displayName+" / "+"Entity Detail";
											}
											...
											if(fldSpan == 2) {
												%>
					                            <li class="d1" lang="<%=PortalUtils.filterValue(tabName)%>" title="<%=displayTitle%>" id="<%=isCustom%>" style="display: <%=display%>"><span><strong><%=LanguageUtil.getString(displayName)%></strong></span></li>
												<%
											} else {
												%>
					                            <li class="d2 fltLeft" lang="<%=PortalUtils.filterValue(tabName)%>" title="<%=displayTitle%>" id="<%=isCustom%>" style="display: <%=display%>"><span><strong><%=LanguageUtil.getString(displayName)%></strong></span></li>
												<%
											}
										}
									%>
